...
def compute_bundle(squad_df,market_df,player_row,budget_m,wage_capacity_m,model_pipe=None):
    ...
    return {"fit":float(fit),"risk":float(risk),"decision":decision,"uplift":uplift,
            "value_per_euro":float(value_per_euro),"p_success":p_success,
            "expected_uplift":float(expected_uplift),"risk_adjusted_value":float(rav)}

def shortlist_top_targets(squad_df,market_df,position,budget_m,wage_capacity_m,top_n=5,min_minutes=400,model_pipe=None):
    candidates = market_df[(market_df["position"]==position)&(market_df["minutes"]>=min_minutes)].copy()
    rows = []
    for _,p in candidates.iterrows():
        b = compute_bundle(squad_df,market_df,p,budget_m,wage_capacity_m,model_pipe=model_pipe)
        rows.append({"name":p["name"],"age":int(p["age"]),"position":p["position"],
            "market_value_m":float(p["market_value_m"]),"estimated_wage_m":float(p["estimated_wage_m"]),
            "injury_risk":float(p["injury_risk"]),"fit_score":b["fit"],"financial_risk":b["risk"],
            "uplift":float(b["uplift"]["uplift"]),"uplift_low":float(b["uplift"]["uplift_lower"]),
            ...
            "recommendation":b["decision"]["recommendation"],"decision_score":float(b["decision"]["decision_score"])})
    out = pd.DataFrame(rows)
    if len(out)==0: return out
    out = out[out["market_value_m"]<=budget_m]
    out = out[out["estimated_wage_m"]<=wage_capacity_m]
    return out.sort_values(by=["risk_adjusted_value","fit_score"],ascending=False).head(top_n).reset_index(drop=True)

def train_club_model_cached(squad_df,market_df,random_state=42):
