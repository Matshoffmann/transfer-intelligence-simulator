...
TOOLS = [
    {"name":"simulate_player_transfer",
     "description":"Runs a full transfer simulation for a single player vs the club squad. Returns fit, uplift, financial risk, ML probability, recommendation.",
     "parameter_definitions":{"player_name":{"description":"Player name from the market.","type":"str","required":True}}},
    {"name":"get_shortlist_for_position",
     "description":"Returns ranked shortlist of best transfer targets for a position under budget/wage constraints.",
     "parameter_definitions":{
         "position":{"description":"One of: GK CB FB DM CM AM ST","type":"str","required":True},
         "top_n":{"description":"Number of candidates (default 3)","type":"int","required":False},
         "max_value_m":{"description":"Max market value filter in M€","type":"float","required":False}}},
    {"name":"compare_two_players",
     "description":"Head-to-head comparison of two players with risk-adjusted verdict.",
     "parameter_definitions":{
         "player_a":{"description":"First player name","type":"str","required":True},
         "player_b":{"description":"Second player name","type":"str","required":True}}},
    {"name":"get_squad_overview",
     "description":"Returns squad depth and average performance per position. Use to identify weaknesses.",
     "parameter_definitions":{}},
]
...
def _tool_simulate(player_name, squad_df, market_df, model_pipe, budget, wage):
    ...
    if p is None: return {"error": f"Player '{player_name}' not found."}
    b = compute_bundle(squad_df, market_df, p, budget, wage, model_pipe=model_pipe)
    return {"player_name":p["name"],"position":p["position"],"age":int(p["age"]),
            "market_value_m":round(float(p["market_value_m"]),2),
            "fit_score":round(float(b["fit"]),2),"uplift":round(float(b["uplift"]["uplift"]),4),
            ...
            "risk_level":b["decision"]["risk_level"],"reasons":b["decision"]["reasons"]}
...
def _tool_shortlist(position, squad_df, market_df, model_pipe, budget, wage, top_n=3, max_value_m=None):
    ...
    eff_budget = min(budget, max_value_m) if max_value_m else budget
    sl = shortlist_top_targets(squad_df, market_df, pos, eff_budget, wage, top_n=top_n, min_minutes=300, model_pipe=model_pipe)
    if len(sl)==0: return {"error":f"No feasible {pos} candidates under current constraints."}
...
def _tool_compare(pa, pb, squad_df, market_df, model_pipe, budget, wage):
    ...
    if "error" in ra or "error" in rb: return {"error": ra.get("error") or rb.get("error")}
    verdict = ra["player_name"] if ra["risk_adjusted_value"]>rb["risk_adjusted_value"] else (rb["player_name"] if rb["risk_adjusted_value"]>ra["risk_adjusted_value"] else "tie")
...
def _tool_squad(squad_df):
    s = squad_df.groupby("position").agg(count=("name","count"),avg_perf=("performance_index","mean"),avg_age=("age","mean")).reset_index().sort_values("avg_perf")
    return {"squad_size":int(len(squad_df)),"position_summary":s.round(3).to_dict(orient="records"),
            "weakest_position":s.iloc[0]["position"],"strongest_position":s.iloc[-1]["position"]}
...
def _dispatch(tool_name, args, squad_df, market_df, model_pipe, budget, wage):
    try:
        if tool_name=="simulate_player_transfer":
            ...
            r = _tool_compare(args.get("player_a",""), args.get("player_b",""), squad_df, market_df, model_pipe, budget, wage)
...
def run_scouting_agent(query, club, squad_df, market_df, model_pipe, transfer_budget, wage_capacity, history=None):
    ...
    system_prompt = f"""You are a senior football transfer analyst for {club}.
Budget: {transfer_budget}M€ | Wage capacity: {wage_capacity}M€/year.
Always use tools to ground answers in data. Respond as a concise professional scouting brief (150-250 words).
Lead with the recommendation, cite exact numbers (fit score, expected uplift, P(success), financial risk).
End with one concrete next step for the Sporting Director."""
    ...
    if not final.strip(): final = "The agent completed analysis but produced no text. Try rephrasing."
